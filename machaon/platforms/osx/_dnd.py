...
TypeFileURL = AppKit.NSFilenamesPboardType
OpNone = AppKit.NSDragOperationNone
OpCopy = AppKit.NSDragOperationCopy


PAD_OPENED = 1
PAD_CLOSED = 2

class DNDPad(AppKit.NSWindow):
    dropped = objc.ivar("dropped")

    def init(self):
        frame = ((200.0, 300.0), (250.0, 500.0))    
        self.initWithContentRect_styleMask_backing_defer_(
            frame, AppKit.NSClosableWindowMask | AppKit.NSTitledWindowMask | AppKit.NSResizableWindowMask,
            2, False
        )
        self.setLevel_(3)
        self.setTitle_ ('')
    ...
    def draggingEntered_(self, sender):
        ...
        if TypeFileURL in types:
            opType = OpCopy
            self.setBackgroundColor_(AppKit.NSColor.selectedTextBackgroundColor())
        return opType
    ...
    def keyDown_(self, event):
        if event.characters() == "d":
            ...
            array = AppKit.NSArray.arrayWithObject_(event)
    ...
    def toggle_show(self, toggle=None):
        ...
        if self.isopened:
            ...
            self.makeKeyAndOrderFront_(AppKit.NSApp)
...
class DNDPadDelegate(AppKit.NSObject):
...
def get_tk_ns_window(fallback=False):
    ...
    for wnd in reversed(AppKit.NSApp.windows()):
