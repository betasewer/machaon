from machaon.parser import CommandParser, OPT_METHOD_TARGET, OPT_METHOD_EXIT
...
def test_add_arg():
    ...
    assert [x for x in p.argnames.keys()] == ["target", "strict"]
...
def test_variable_posit():
    p = build_parser(
        option("name"),
        option("target", variable=True),
        option("--log", "-l", value="files.txt"),
        option("--strict", flag=True)
    )
    assert p.do_parse_args(["watanabe", "input.txt", "output.txt", "-l", "--strict"]) == {
        'name' : 'watanabe',
...
def test_remainder():
    ...
    assert p.do_parse_args("-v select * from table --age 23".split()) == {
...
def test_defaults(): 
    p = build_parser(
        ...
        option("--depth", "-d", const=3, default=1, valuetype=int),
    )
...
def test_compound_rows():  
    ...
    def parse_and_recompose(cmd):
        ...
        for cmdrow in p.generate_command_rows(cmd):
            rows.append([x.names[1] if not isinstance(x,str) else x for x in cmdrow])
        return rows
